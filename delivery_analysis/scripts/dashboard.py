...
# -------------------------------
# 1️⃣ Paths and SQLite connection
# -------------------------------
data_csv = r"C:\MyProjects\delivery_analysis\Data\delivery_data.csv"
db_path = r"C:\MyProjects\delivery_analysis\Data\delivery_data.db"

# Create SQLite database if it doesn't exist
conn = sqlite3.connect(db_path)

# Load CSV into SQLite table (replace if exists)
df_csv = pd.read_csv(data_csv)
df_csv.to_sql("DeliveryData", conn, if_exists="replace", index=False)

# Read table from SQLite
df = pd.read_sql("SELECT * FROM DeliveryData", conn)
conn.close()
...
def update_charts(selected_cities):
    ...
    avg_delay_fig = px.bar(
        filtered_df.groupby('city')['delay_minutes'].mean().reset_index(),
    ...
    scatter_fig = px.scatter(
        filtered_df, x='distance_km', y='weight_kg',
        color='carrier', title='Distance vs Weight by Carrier'
    )
    
    # Correlation Heatmap
    heatmap_fig = px.imshow(
        filtered_df[['delay_minutes','distance_km','weight_kg']].corr(),
